...
def report_290(branches, start_date, end_date):
    conn = psycopg2.connect(dbname=db_name, host='172.16.10.22', port='5432',
                            user=db_username, password=db_password)
    ...
    df1.to_excel(os.path.join(saving_path, str(df1[df1.columns[1]].iloc[0]) + '.xlsx'), index=False)
    print('Saved')
    return df1
...
if __name__ == '__main__':
    ...
    for i in range(15, len(all_branches_ids)):
        current_stores = list(get_store_ids_by_branch_id(all_branches_ids['Номера филиалов'].iloc[i])['Номер магазина'])
        ...
        df = report_290(current_stores, datetime.datetime.strptime(start_date, "%Y-%m-%d").strftime("%d.%m.%Y"), datetime.datetime.strptime(end_date, "%Y-%m-%d").strftime("%d.%m.%Y"))
        end_time = time.time()
